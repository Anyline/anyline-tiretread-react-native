...
const getLinkingError = () =>
  `The package 'anyline-tire-tread-react-native-module' doesn't seem to be linked. Make sure: \n\n` +
...
export function scan(config: string, tireWidth?: number): Promise<string> {
  const scanPromise =
    tireWidth !== undefined
      ? AnylineTtrMobileWrapperReactNative.startTireTreadScanActivity(
          config,
          tireWidth
        )
      : AnylineTtrMobileWrapperReactNative.startTireTreadScanActivity(
...
function scanInternal(
  config: string,
  tireWidth?: number
): Promise<{ uuid: string; cameraDirection?: string }> {
  ...
  return scanInternal(config, tireWidth)
    .then((result) => {
      ...
      return {
        ...
        cameraDirection:
          (result.cameraDirection as CameraDirection) ||
          capturedCameraDirection,
      };
    })
    .catch((error) => {
      subscription?.remove();
